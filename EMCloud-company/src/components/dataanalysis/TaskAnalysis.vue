  ...
  <div class="ai-wrapper">
    <div class="block-title" style="margin-bottom:20px">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>设备任务分析</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      <div class="block-picker">
        ...
        <el-button-group>
          <el-button
            type="primary"
            size="medium"
            @click='getTaskInfoDefault("day",7)'
          >
            近七天任务数据
          </el-button>
          <el-button
            type="primary"
            size="medium"
            @click='getTaskInfoDefault("day",30)'
          >
            近30天任务数据
          </el-button>
          <el-button
            type="primary"
            size="medium"
            @click='getTaskInfoDefault("mouth",180)'
          >
            近半年任务数据
          </el-button>
        </el-button-group>
...
import {
  getProTaskByDay, getProTaskByMouth,
  getProType,
  getTaskByDay,
  getTaskByMouth,
  getTaskTypeByDay,
...
export default {
  ...
  methods: {
    ...
    getTaskInfoDefault(type, days){//默认
      this.getDate(days)
      this.getTask(type)
      this.getTaskType(type)
    },
              ...
              if (res) {
                ...
                for (let i = 0; i < this.xAxisData.length; i++) {//不同类型
                  if (time.indexOf(this.xAxisData[i]) !== -1) {
                    this.yAxisData[i] = res[time.indexOf(this.xAxisData[i])].count;
    ...
    getTaskType(type) {
      ...
      if(type==='day') {//获取不同类型的异常；按天获取
        ...
        getTaskTypeByDay(params)
          .then(res => {
            if (res) {
              console.log(res)
              for (let i = 0; i < res.length; i++) {
                this.legendData.push(res[i].name)//res[i].name是产品的类型
                let dataList = res[i].data;//res[i].data是每个产品类型的相关数据，包括时间和异常数量
        ...
        getTaskTypeByMouth(params)
          .then(res => {
            if (res) {
              console.log(res)
              for (let i=0;i<res.length;i++){
                this.legendData.push(res[i].name)
                let dataList = res[i].data;
                console.log(dataList)
                let time=[];
                for (let j=0;j<dataList.length;j++) {
